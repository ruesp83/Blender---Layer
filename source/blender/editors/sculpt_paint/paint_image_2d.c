...
static void paint_2d_lift_smear(ImBuf *ibuf, ImBuf *ibufb, int *pos, short lock_alpha)
{
	...
	for (a = 0; a < tot; a++)
		IMB_rectblend(ibufb, ibufb, ibuf, NULL, NULL, 0, region[a].destx, region[a].desty,
		              ...
		              region[a].width, region[a].height, IMB_BLEND_COPY_RGB, lock_alpha);
}
			...
			for (ty = tiley; ty <= tileh; ty++) {
				for (tx = tilex; tx <= tilew; tx++) {
					...
					IMB_rectblend(s->canvas, tmpbuf, frombuf, mask,
					              ...
					              origx, origy,
					              region[a].srcx, region[a].srcy,
								  region[a].width, region[a].height, blend, ((ImageLayer *)imalayer_get_current(s->image))->locked);
				}
			}
		}
		else {
